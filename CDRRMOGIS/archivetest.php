        ...
        <?php
           $sql ='SELECT 
            ...
            while($row = $result->fetch_array())
            {
                ...
                $deadpercentile = (($totaldead - 0)/($totalpopulation - 0));
                $missingpercentile = (($totalmissing - 0)/($totalpopulation - 0));
                $injuredpercentile = (($totalinjured - 0)/($totalpopulation - 0));
                $dmgpercentile = (($totaltotaldmg - 0)/($totalhouses - 0));
                $partialpercentile = (($totalpartialdmg - 0)/($totalhouses - 0));

                $totalpercentile = (($deadpercentile * 0.45 ) + ($missingpercentile * 0.2) + ($injuredpercentile * 0.05) + ($dmgpercentile * 0.20) + ($partialpercentile * 0.10))*100;

                ?> ['<?php echo $row['ID']; ?>','<?php echo $row['LAT']; ?>','<?php echo $row['LNG']; ?>','<?php echo $row['RADIUS']; ?>','<?php echo $row['COLOR']; ?>','<?php echo $row['STARTED']; ?>','<?php echo $row['JSON_FACTORS']; ?>','<?php echo $row['NICKNAME']; ?>','<?php echo $row['ENDED']; ?>','<?php echo $row['TYPENAME']; ?>','<?php echo round($totalpercentile,2) ?>'],
                <?php
            }    
            ?>
