...
<div class="container"> <!--Content starts here-->
    <div class="page-header">
        <h1>
            Declare a Disaster
            <small> happened in <br />Brgy. <?php echo $result_BRGYNAME . ', ' . $result_DISTRICTNAME . ', ' . $result_CITYNAME; ?> City.</small>
        </h1>
    </div>
	...
	<div class="row">
		...
		<div class="col-lg-6">
			<form id="BrgyDeclareDisasterForm" method="post"  action="library/form/BrgyDeclareDisasterForm.php">
...
<script>
    ...
    function initAutocomplete() {
        <?php 
		$coordinatesphp = "";
		$sql = "SELECT LAT, LNG FROM barangay_coordinates WHERE BARANGAY = " . $_SESSION['USER_BRGY'];
        $result = $db->connection->query($sql);
        $count = mysqli_num_rows($result);
        while($row = $result->fetch_assoc()){
        ...
        // evac_marker = new google.maps.Marker({
            // position: polycenter(coordinates),
            // map: map,
            // title: 'Your Location.',
            // draggable: true,
        ...
        // map.addListener('click', function(event) {
			// var isInside = google.maps.geometry.poly.containsLocation(event.latLng, brgyPolyline);
        ...
        // evac_marker.addListener('dragend', function(event) {
			...
			// }
			// else {
				// evac_marker.setPosition(polycenter(coordinates));
				// evac_marker.setAnimation(google.maps.Animation.BOUNCE);
				// geocodeLatLng(geocoder,event.latLng);
			// }
        // });
