 ...
 */
	...
	$sql = 'UPDATE evacuation_report SET ISVERIFIED=0 WHERE ID=' . $form_ID;

	if($db->connection->query($sql)) {
		echo 'success';
		console_log('USER(' . $session_USER_USERNAME . ') moved the evacuation report(' . $form_ID . ') to pending list.', '../../system/log.txt');
	}
	else {
		echo 'error';
	}
