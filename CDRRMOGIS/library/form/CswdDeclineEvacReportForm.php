 ...
 */
	...
	$sql = 'UPDATE evacuation_report SET ISVERIFIED=2 WHERE ID=' . $form_ID;

	if($db->connection->query($sql)) {
		echo 'success';
		console_log('USER(' . $session_USER_USERNAME . ') declined the evacuation report(' . $form_ID . ')', '../../system/log.txt');
	}
	else {
		echo 'error';
	}
