 ...
 */
	...
	$sql = 'UPDATE disaster_reports SET ISVERIFIED=1 WHERE ID=' . $form_ID;

	if($db->connection->query($sql)) {
		echo 'success';
		console_log('USER(' . $session_USER_USERNAME . ') accepted the disaster report(' . $form_ID . ')', '../../system/log.txt');
