...
$totalservedpersons = '0';
        ...
        while($row = $result->fetch_assoc()){
            $totaldeaths += $row['CSLTDEAD'];
            $totalinjured += $row['CSLTINJURED'];
            $totalmissing += $row['CSLTMISSING'];
        }
        ...
        while($row2 = $result2->fetch_assoc()){
            $totalassistance += $row2['DSWD'];
            $totalassistance += $row2['LGU'];
            $totalassistance += $row2['NGO'];
        }
        ...
        while($row3 = $result3->fetch_assoc()){
            $totalservedfamily += $row3['SRVFAMILIES'];
            $totalservedpersons += $row3['SRVPERSONS'];
        }      
    ...
    {
        $totaldeaths = $totaldeaths / $count;
        $totalinjured = $totalinjured / $count;
        $totalmissing = $totalmissing / $count;
        
    }
    else if($count2 != 0)
    {
        $totalassistance = $totalassistance / $count2; 
    }
    else if($count3 != 0)
    {
        $totalservedpersons = $totalservedpersons / $count3;
        $totalservedfamily = $totalservedfamily / $count3;
    }
